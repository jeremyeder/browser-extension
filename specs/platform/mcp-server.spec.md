...
Each credential sidecar receives `AMBIENT_API_URL`, exact HTTPS
`AMBIENT_CP_TOKEN_URL`, the pinned `AMBIENT_CP_TOKEN_CA`, the pod-bound `AMBIENT_WORKLOAD_IDENTITY_PATH`, and an
exact-Session `AMBIENT_CAPABILITY_REFRESH_CREDENTIAL`. It authenticates with
JSON `POST /token` and hard-coded `grant=runtime-only`, then refreshes before
the two-minute runtime credential expires. It MUST NOT use the removed
RSA-OAEP encrypted-Session `GET /token` protocol or receive ensure authority.
...
│     AMBIENT_API_URL, AMBIENT_CP_TOKEN_URL, AMBIENT_CP_TOKEN_CA
│     AMBIENT_WORKLOAD_IDENTITY_PATH, AMBIENT_CAPABILITY_REFRESH_CREDENTIAL
│     authenticated runtime-only JSON POST exchange; no ensure capability
│
