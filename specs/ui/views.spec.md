...
### Scenario: Trusted bootstrap is not attributed to the user

- GIVEN Session history contains a trusted `bootstrap` SessionMessage
- AND Session history contains a human-authored task
- WHEN the Chat tab or persistent chat sidebar renders the conversation
- THEN the bootstrap is omitted from ordinary human-authored message bubbles
- AND it is never labeled or styled as `You`
- AND an explicit collapsed `ACP startup context` affordance SHALL expose it for inspection
- AND the genuine human task remains visible exactly once as a user-authored message

### Scenario: Legacy startup context remains compatible

- GIVEN a legacy Session stores platform startup context as a `user` SessionMessage
- WHEN that complete row exactly matches the trusted startup-context digest retained for the Session
- THEN the Ambient UI may omit it from ordinary human-authored bubbles or expose it as `ACP startup context`
- AND unknown, untrusted, or merely similar user text remains visible as user-authored content
